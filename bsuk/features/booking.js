...
var { createElement: h, useState, useCallback } = React;
...
window.PrayerBookingTab = function PrayerBookingTab({ api, calendar, onBooked, feat }) {
  ...
  const isSlotTaken    = (date, time) => (calendar||[]).some(b => b.date === date && (b.slot||b.time) === time && b.isBooked);
  ...
  const handleSlotSelect = (time) => {
    if (form.date && isSlotTaken(form.date, time)) {
      ...
      triggerError(`🚫 "${time} Prayer" on ${window.formatDate(form.date)} is already booked.\n\nPlease choose a different date or slot.`);
      return;
    }
    setError("");
    setForm(f => ({ ...f, time }));
  ...
  const handleBook = async () => {
    ...
    if (isSlotTaken(date, time)) {
      ...
      triggerError(`🚫 "${time} Prayer" on ${window.formatDate(date)} is already booked.\n\nPlease choose a different date or slot.`);
      return;
    }
