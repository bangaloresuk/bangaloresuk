...
window.PhotoGalleryTab = function PhotoGalleryTab({ api, sukKey }) {
  var [upload,    setUpload]    = useState({ caption:"", uploader:"", file:null, preview:null });
  var [uploading, setUploading] = useState(false);
  var [msg,       setMsg]       = useState("");
  var [photos,    setPhotos]    = useState([]);
  var [loading,   setLoading]   = useState(true);
  var [photoErr,  setPhotoErr]  = useState("");
  var [showUp,    setShowUp]    = useState(false);

  var loadPhotos = useCallback(async () => {
    setLoading(true); setPhotoErr("");
    try {
      var res = await api.photos.getAll();
      setPhotos(res.data || []);
    } catch(e) {
  ...
  var handleUpload = async () => {
    ...
    reader.onload = async (ev) => {
      try {
        ...
        var res = await api.photos.upload(
  ...
  return h("div", { style:{ display:"flex", flexDirection:"column", gap:16 } },
    ...
    h("div", { className:"card" },
      h("button", {
        onClick: () => { setShowUp(!showUp); setMsg(""); },
        style:{ width:"100%", padding:11, border:"1px solid rgba(59,130,246,0.3)", borderRadius:11,
          background:showUp?"rgba(239,246,255,0.9)":"rgba(239,246,255,0.5)", color:"#1e3a8a",
          fontWeight:700, fontSize:13, cursor:"pointer", display:"flex", alignItems:"center",
          justifyContent:"center", gap:8 }
      }, showUp ? "▲  Close Upload" : "📸  Upload a Photo"),

      showUp && h("div", { className:"fade-in", style:{ marginTop:14, display:"flex", flexDirection:"column", gap:13 } },
        h("div", null,
          ...
          h("label", {
            ...
            upload.preview
              ...
              : h("div", null,
                  ...
                  h("div", { style:{ fontSize:11, color:"rgba(29,78,216,0.35)", marginTop:4 } }, "JPG, PNG up to 5MB")
                ),
            h("input", {
        ...
        h("div", null,
          ...
          h("input", { className:"divine-input", placeholder:"e.g. Morning prayer at home",
    ...
    h("div", { className:"card" },
      h("div", { style:{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 } },
        h("div", { style:{ fontFamily:"'Cinzel',serif", color:"#1e3a8a", fontSize:14, fontWeight:700 } },
          "🌸 Gallery",
          !loading && photos.length > 0 && h("span", { style:{ fontSize:11, color:"rgba(29,78,216,0.45)", fontWeight:600, marginLeft:8 } },
      ...
      loading
        ? h("div", { style:{ textAlign:"center", padding:"24px", color:"rgba(29,78,216,0.4)", fontSize:13 } }, "⏳ Loading gallery...")
        : photoErr
          ? h("div", { style:{ textAlign:"center", padding:"20px", color:"#b91c1c", fontSize:13 } },
              ...
              h("button", { onClick:loadPhotos,
                style:{ marginTop:10, padding:"7px 14px", border:"1px solid #fca5a5",
                  borderRadius:8, background:"#fff", color:"#dc2626", fontWeight:700, fontSize:12, cursor:"pointer" } }, "Try Again"))
          : photos.length === 0
            ...
            : h("div", { style:{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:10 } },
                photos.map((p, i) =>
                  h("div", { key:p.id||i, style:{ borderRadius:12, overflow:"hidden",
                    border:"1px solid rgba(59,130,246,0.15)", background:"rgba(239,246,255,0.4)",
                    ...
                    (p.caption||p.uploader||p.date) && h("div", { style:{ padding:"8px 10px 4px" } },
                      p.caption && h("div", { style:{ fontSize:11, color:"#1e3a8a", fontWeight:600, lineHeight:1.4, marginBottom:2 } }, p.caption),
                      p.uploader && h("div", { style:{ fontSize:10, color:"rgba(29,78,216,0.45)" } }, "🙏 " + p.uploader),
                      p.date && h("div", { style:{ fontSize:10, color:"rgba(29,78,216,0.3)", marginTop:2 } }, window.cleanPhotoDate(p.date))
                    ),
