...
//  Props: { bookings, satsangBookings, !loading }
// ============================================================

"use strict";

var { createElement: h, useState, useMemo } = React;
...
window.AllBookingsView = function AllBookingsView({ api, sukKey }) {
  ...
  const filtered = useMemo(() => allItems.filter(b => {
    if (typeTab==="prayer"  && b._type && b._type!=="prayer")  return false;
    if (typeTab==="satsang" && (b._type&&b._type!=="satsang" || !b._type)) return false;
    if (isSearching) {
  ...
  const renderCard = (b) => {
    if (b._type === "prayer") {
      const sc = window.SLOT_STYLE[b.slot||b.time] || window.SLOT_STYLE["Morning"];
      return h("div", {
        key: b.id,
        ...
        h("div", { style:{ padding:"12px 14px" } },
          ...
          h("div", { style:{ fontSize:13, color:sc.color, fontWeight:700, marginBottom:3 } },
            `${sc.icon} ${b.slot||b.time} · ${b.initial ? "🙏 " + b.initial : ""}`),
  ...
  return h("div", { style:{ display:"flex", flexDirection:"column", gap:10 } },
    ...
    h("div", { className:"card", style:{ padding:"14px 16px" } },
      ...
      !isSearching && h("div", { style:{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14 } },
        h("button", { onClick:()=>monthIdx>0&&setActiveYM(allMonths[monthIdx-1]),
          disabled:monthIdx<=0,
        ...
        h("button", { onClick:()=>monthIdx<allMonths.length-1&&setActiveYM(allMonths[monthIdx+1]),
      ...
      h("div", { style:{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:6, marginBottom:12 } },
        TYPE_TABS.map(t => {
          ...
          return h("button", { key:t.id, onClick:()=>setTypeTab(t.id),
            ...
            h("div", { style:{ fontSize:10, marginTop:1, opacity:active?0.85:0.55,
              color:active?"#fff":"#1d4ed8", fontFamily:"sans-serif", fontWeight:700 } }, counts[t.id])
          );
        })
      ),
    ...
    loading
      ? h("div", { style:{ display:"flex", flexDirection:"column", gap:10 } },
          h(window.SkeletonCard, { rows:3 }), h(window.SkeletonCard, { rows:3 }), h(window.SkeletonCard, { rows:2 }))
      : filtered.length === 0
        ? h("div", { className:"card", style:{ textAlign:"center", padding:"40px 0" } },
            ...
            search && h("button", { onClick:()=>setSearch(""),
              style:{ marginTop:10, padding:"7px 16px", borderRadius:20,
                ...
                color:"#1d4ed8", fontSize:12, fontWeight:700, cursor:"pointer" } }, "Clear search")
          )
            ...
            sortedDates.map(date => {
              ...
              return h("div", { key:date },
                h("div", { style:{ display:"flex", alignItems:"center", gap:8, margin:"10px 0 6px" } },
                  ...
                  h("div", { style:{ flex:1, height:1, background:"linear-gradient(90deg,rgba(59,130,246,0.12),transparent)" } })
                ),
