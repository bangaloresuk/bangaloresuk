...
// queryParams: optional key-value pairs appended to the URL (e.g. { sheetName, action })
// GAS reads these reliably via e.parameter regardless of body parsing.
// NOTE: Google Apps Script POST requests often redirect (302). fetch() follows
// but converts POST → GET, dropping the body. Fix: put all critical params in
// the URL so they survive the redirect as query parameters.
async function post(body, queryParams = {}) {
  const allParams = { ...body, apiKey: _apiKey, ...queryParams }
  ...
  // GAS sometimes returns HTML on error instead of JSON — handle gracefully
